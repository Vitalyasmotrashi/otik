...
{
\includegraphics[width=0.3\linewidth,valign=c]{06_channel}\centering
...
\subsection{Защита от помех}
\begin{frame}{\insertsubsection}

Защита от помех "--- добавление избыточности соответственно помехам:
$|code(x)|\to |x| + \Delta$.

Если известен источник $X \ni x$ и~изначальный код избыточен,
его~избыточность желательно вначале удалить: 
$|code(X)|\to I(X) + \Delta$.

Величина добавленной избыточности $\Delta$
соответствует характеристикам канала.
...
\includegraphics[height=12ex,valign=t]{06_Entropy-mutual-information-relative-entropy-relation-diagram}
&
...
% ~~~
&\hspace{-1.3em}%
\rlap{\includegraphics[width=8em,valign=t]{06_Entropy-mutual-information-relative-entropy-relation-diagram}}
\end{tabular}
% \bigskip
...
\input{\SRCROOTPATH/pres-sections/05_bin_symm_channel.tex}
...
\input{\SRCROOTPATH/pres-sections/06_hamming.tex}
\input{\SRCROOTPATH/pres-sections/08_rs_poly.tex}
\input{\SRCROOTPATH/pres-sections/08_rs_z5.tex}
