...
\node[wideblock,  espdblackcenter, espdprocess, above=of W] (init_1) {\underline{ѕервична€ инициализаци€:} $i = 1$,
        \mbox{словарь $\big\{(0, \hex{00}), (1, \hex{01}),  \ldots  (255, \hex{FF})\big\}$}
...
\node[wideblock,  espdblackcenter, espdprocess, left=of init_1] (init_i) {\underline{»нициализаци€ дл€ нового $i$:}\hfill
    ...
    чтение одного символа $c_i$ и $E_L = N_L = c_i$
};
...
\node[text width=9em, text badly centered, inner xsep = 0em, anchor=south ] (totalcomment) at (EL1|-find.south) {Ќомер $N_L$ кодирует только $E_{L}$};
{
